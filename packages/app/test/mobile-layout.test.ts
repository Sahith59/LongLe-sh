...
describe('delegation mobile layout contract', () => {
  ...
  it('keeps the Help sheet modal, keyboard-contained, and focus-restoring', () => {
    ...
    expect(app).toMatch(/className="help-sheet-header"[\s\S]*?className="help-sheet-scroll"/)
    expect(css).toMatch(/\.help-sheet\s*{[\s\S]*?grid-template-rows:[\s\S]*?overflow: hidden;/)
